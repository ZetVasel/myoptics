...
@section('main')
	...
	<h1>
		...
		<a href="/master/articles/add" class="glyphicon glyphicon-plus-sign"></a>{{ $title }} <span class="label label-default">{{ $articles_count }}</span>
	</h1>
			...
			<table class="table">
				...
				<tr>
					<td style="vertical-align: middle">
						<input name="check[]" value="{{ $post->id }}" type="checkbox"><a class="black_link" href="/master/articles/edit/{{ $post->id }}">{{{ $post->name }}}</a><br>
						<small style="padding-left: 22px;">{{ $post->created_at }}</small>
					</td>
					<td style="text-align: right;">
						<div class="btn-group">
							<a title="Редактировать" href="/master/articles/edit/{{ $post->id }}" class="btn btn-success"><span class="glyphicon glyphicon-pencil"></span></a>
							<a title="Открыть в новом окне" target="_blank" href="/articles/{{ $post->slug }}" class="btn btn-primary"><span class="glyphicon glyphicon-new-window"></span></a>

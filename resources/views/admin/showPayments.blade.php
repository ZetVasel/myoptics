...
@section('main')
	...
	<h1>
		...
		<a href="/master/payment/add" class="glyphicon glyphicon-plus-sign"></a>{{ $title }} <span class="label label-default">{{ $payment_count }}</span>
	</h1>
			...
			<table class="table">
				...
				<tr>
					<td style="vertical-align: middle">
						<input name="check[]" value="{{ $post->id }}" type="checkbox"><a class="black_link" href="/master/payment/edit/{{ $post->id }}">{{{ $post->name }}}</a><br>
					</td>
					<td style="text-align: right;">
						<div class="btn-group">
							<a title="Редактировать" href="/master/payment/edit/{{ $post->id }}" class="btn btn-success"><span class="glyphicon glyphicon-pencil"></span></a>

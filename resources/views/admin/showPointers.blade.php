...
@section('main')
    ...
    <h1>
        ...
        <a href="/master/pointers/add" class="glyphicon glyphicon-plus-sign"></a>{{ $title }} <span class="label label-default">{{ $pointers_count }}</span>
    </h1>
    ...
    @if( count($pointers)>0 )
        ...
        <table class="table">
            ...
            @foreach ($pointers as $post)
                <tr>
                    <td style="vertical-align: middle">
                        <input name="check[]" value="{{ $post->id }}" type="checkbox"><a class="black_link" href="/master/pointers/edit/{{ $post->id }}">{{{ $post->address }}}</a><br>
                        <small style="padding-left: 22px;">{{ $post->created_at }}</small>
                    </td>
                    <td style="text-align: right;">
                        <div class="btn-group">
                            <a title="Редактировать" href="/master/pointers/edit/{{ $post->id }}" class="btn btn-success"><span class="glyphicon glyphicon-pencil"></span></a>
